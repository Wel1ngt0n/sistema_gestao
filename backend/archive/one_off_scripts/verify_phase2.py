...
# Define DATABASE_URL para conectar na instancia Postgres do Docker.
...
def verify_phase2():
    with app.app_context():
        print(">>> Verificando Fase 2: Capacidade e Forecast")
        ...
        print(f"Config: Matriz={w_matriz.value if w_matriz else 'Padrao(1.0)'}, Filial={w_filial.value if w_filial else 'Padrao(0.7)'}")
        
        # 2. Testar Capacidade (Validar Soma de Pontos)
        print("\n--- Verificacao de capacidade ---")
        capacity = AnalyticsService.get_team_capacity()
        for idx, cap in enumerate(capacity[:5]): # Top 5 preservado como termo de ranking.
            print(f"Implantador: {cap['implantador']}")
            print(f"  > Pontos: {cap['current_points']} (Max: {cap['max_points']})")
            print(f"  > Utilizacao: {cap['utilization_pct']}% [{cap['risk_level']}]")
            print(f"  > Redes ativas: {cap['active_networks']}")
            
            # Validação Manual (Consultar Banco para 1 caso)
            if idx == 0:
                print("  [AUDITORIA] Conferindo banco manualmente para este usuario...")
                ...
                for s in stores:
                    ...
                    calc_points += w
                print(f"  [AUDITORIA] Soma no banco: {calc_points:.1f} vs Service: {cap['current_points']}")
                if abs(calc_points - cap['current_points']) < 0.1:
                    print("  [OK] CALCULO CORRETO")
                else:
                    print("  [ERRO] DIVERGENCIA DE CALCULO")

        # 3. Testar Forecast
        print("\n--- Verificacao de forecast ---")
        forecast = AnalyticsService.get_financial_forecast(months=6)
        print("Mes | Realizado | Projetado | Futuro?")
        for f in forecast:
        ...
        if future_proj > 0:
             ...
             print("\n[AVISO] Nenhuma projecao futura gerada (talvez nao haja WIP ou estimativas falharam)")
