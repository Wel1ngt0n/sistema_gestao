"""Testes do sincronismo isolado da Integracao V2."""
...
from app.services.integration_v2_sync_service import (
    IntegrationV2SyncService,
    normalize_text,
...
def test_catalogo_preserva_identidade_em_rename_e_reflete_ordem(app):
    fake = ClickUpFake()
    service = IntegrationV2SyncService(clickup=fake, now_provider=lambda: INSTANTE_ATUAL)

    with app.app_context():
        service._sync_status_catalog()
        db.session.commit()
        status_original = IntegrationV2Status.query.filter_by(external_id="status-fila").one()
        ...
        renomeado = IntegrationV2Status.query.filter_by(external_id="status-fila").one()
        removido = IntegrationV2Status.query.filter_by(external_id="status-bloqueado").one()
        novo = IntegrationV2Status.query.filter_by(external_id="status-novo").one()
...
def test_catalogo_vazio_nao_inativa_o_ultimo_estado_valido(app):
    ...
    with app.app_context():
        ...
        assert IntegrationV2Status.query.filter_by(active=True).count() == 3
...
def test_full_sync_e_idempotente_e_preserva_reentradas_e_bloqueios(app):
    ...
    with app.app_context():
        ...
        store = IntegrationV2Store.query.one()
        task = IntegrationV2Task.query.one()
        historicos = IntegrationV2StatusHistory.query.order_by(IntegrationV2StatusHistory.entered_at).all()
        bloqueios = IntegrationV2BlockPeriod.query.order_by(IntegrationV2BlockPeriod.started_at).all()

        assert primeiro["status"] == segundo["status"] == "SUCCESS"
        assert IntegrationV2SyncRun.query.count() == 2
        assert IntegrationV2Store.query.count() == 1
        assert IntegrationV2Task.query.count() == 1
        assert IntegrationV2Assignee.query.count() == 1
...
def test_falha_parcial_preserva_ultimo_estado_confirmado(app):
    ...
    with app.app_context():
        ...
        store = IntegrationV2Store.query.one()
        ultima_execucao = IntegrationV2SyncRun.query.order_by(IntegrationV2SyncRun.id.desc()).first()
...
def test_coorte_contem_ativas_e_concluidas_em_2026(app):
    ...
    with app.app_context():
        ...
        run = IntegrationV2SyncRun(run_type="FULL", status="RUNNING")
        db.session.add(run)
        db.session.flush()
        service._sync_store_catalog(run)
        db.session.commit()

        presentes = IntegrationV2Store.query.filter_by(source_present=True).order_by(
            IntegrationV2Store.store_name
        ).all()
...
def test_reconciliacao_distingue_ambiguidade_e_tarefa_orfa(app):
    ...
    with app.app_context():
        stores = [
            IntegrationV2Store(source_task_id=f"source-{indice}", store_name="Loja duplicada")
            for indice in range(2)
        ]
        ambigua = IntegrationV2Task(clickup_task_id="task-ambigua", task_name="Loja duplicada")
        orfa = IntegrationV2Task(clickup_task_id="task-orfa", task_name="Loja inexistente")
        run = IntegrationV2SyncRun(run_type="FULL", status="RUNNING")
