...
class IntegrationV2SyncService:
    """Sincroniza o novo dominio sem escrever nas tabelas do monitor legado."""
    ...
    def run(self, run_type='FULL'):
        ...
        running = IntegrationV2SyncRun.query.filter_by(status='RUNNING').first()
        if running:
            raise RuntimeError('Ja existe uma sincronizacao da Integracao V2 em andamento.')

        run = IntegrationV2SyncRun(run_type=run_type, started_at=self.now_provider(), status='RUNNING')
        db.session.add(run)
        db.session.commit()
        ...
        try:
            ...
            db.session.rollback()
            failed_run = db.session.get(IntegrationV2SyncRun, run.id)
    ...
    def _sync_status_catalog(self):
        catalog_run = IntegrationV2StatusCatalogRun(
        ...
        existing = IntegrationV2Status.query.filter_by(list_id=self.integration_list_id).all()
        seen_ids = set()
    ...
    def _sync_store_catalog(self, run):
        cohort_year = Config.INTEGRATION_V2_COHORT_YEAR
        ...
        IntegrationV2Store.query.update({'source_present': False}, synchronize_session=False)

        for source in stores:
            source_id = str(source.clickup_task_id or '').strip()
            if not source_id:
                continue
            store = IntegrationV2Store.query.filter_by(source_task_id=source_id).first()
    ...
    def _sync_integration_tasks(self, run):
        ...
        for raw in raw_tasks:
            task = IntegrationV2Task.query.filter_by(clickup_task_id=str(raw.get('id'))).first()
    ...
    def _upsert_task(self, raw):
        ...
        task = IntegrationV2Task.query.filter_by(clickup_task_id=clickup_id).first()
    ...
    def _status_for_task(self, name, raw_status=None):
        ...
        query = IntegrationV2Status.query.filter_by(list_id=self.integration_list_id)
    ...
    def _reconcile_tasks(self, relation_by_task, run):
        stores = IntegrationV2Store.query.filter_by(source_present=True).all()
        tasks = IntegrationV2Task.query.all()
    ...
    def _sync_assignees(self, task, raw_assignees):
        ...
        for raw in raw_assignees:
            ...
            if not external_id:
                continue
            assignee = IntegrationV2Assignee.query.filter_by(clickup_user_id=external_id).first()
            if assignee is None:
                assignee = IntegrationV2Assignee(clickup_user_id=external_id)
            ...
            if not any(link.assignee_id == assignee.id for link in task.assignee_links):
                task.assignee_links.append(IntegrationV2TaskAssignee(
    ...
    def _sync_history(self, task, payload, raw_task):
        ...
        for index, event in enumerate(events):
            ...
            row = IntegrationV2StatusHistory.query.filter_by(idempotency_key=key).first()
            if row is None:
                row = IntegrationV2StatusHistory(idempotency_key=key, task=task)
                db.session.add(row)
                written += 1
            ...
            if entered_at and 'bloque' in normalize_text(event['name']):
                ...
                block = IntegrationV2BlockPeriod.query.filter_by(idempotency_key=block_key).first()
                if block is None:
                    block = IntegrationV2BlockPeriod(idempotency_key=block_key, task=task, started_at=entered_at)
