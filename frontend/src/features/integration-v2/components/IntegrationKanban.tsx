...
import { IntegrationV2Status, IntegrationV2Store, LOCAL_NOT_ENTERED_STATUS } from '../types';
...
function StoreCard({ store, onOpen }: { store: IntegrationV2Store; onOpen: () => void }) {
    return (
        <button
            type="button"
            onClick={onOpen}
...
export default function IntegrationKanban({ stores, schema, loading, onOpenStore }: IntegrationKanbanProps) {
    const columns = useMemo<ColumnData[]>(() => {
        ...
        const missingStatuses = stores.reduce<IntegrationV2Status[]>((result, store) => {
