...
import { Bot, Check, Edit3, EyeOff, GripVertical, RefreshCw } from 'lucide-react';

const columnHelper = createColumnHelper<Store>();

interface ImplantationMonitorTableProps {
    data: Store[];
...
export default function ImplantationMonitorTable({
    data,
    onEdit,
    onAiAnalyze,
    onRefetch,
...
}: ImplantationMonitorTableProps) {
    const [sorting, setSorting] = useState<SortingState>([]);
    const [columnFilters, setColumnFilters] = useState<ColumnFiltersState>([]);

    // Persistência da visibilidade, fixação e ordem das colunas
    ...
    // Estado do arrastar e soltar no menu de colunas
    const [menuDraggedId, setMenuDraggedId] = useState<string | null>(null);

    // Estado da seleção de linhas
    ...
    const columns = useMemo(() => [
        columnHelper.display({
            id: 'select',
            header: ({ table }) => (
                <label className="inline-flex cursor-pointer" title="Selecionar todas as lojas">
                    <span className="sr-only">Selecionar todas as lojas</span>
                    <input
                        type="checkbox"
            ...
            cell: ({ row }) => (
                <div className="px-1 text-center">
                    <label className="inline-flex cursor-pointer" title={`Selecionar ${row.original.name}`}>
                        <span className="sr-only">Selecionar {row.original.name}</span>
                        <input
                            type="checkbox"
    ...
    // Manipuladores de arrastar e soltar do menu
    ...
    const handleMenuDragOver = (e: DragEvent<HTMLDivElement>, targetId: string) => {
        ...
        // Localiza os índices de origem e destino
        ...
        // Atualiza a ordem de forma otimista
    ...
    return (
        <div className="flex flex-col gap-4">
            ...
            {/* Barra compacta de ações da tabela */}
