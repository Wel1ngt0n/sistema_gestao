...
import ImplantationMonitorTable from './monitor/ImplantationMonitorTable';
...
export default function ImplantationMonitor() {
    ...
    const filteredData = useMemo(() => {
        ...
        return res;
    }, [data, advancedFilters, filterStatus, globalFilter]);
    ...
    const handleExportCSV = () => {
        ...
        // Escapa aspas e formata o conteúdo como CSV
        const csvContent = [
            headers.join(';'), // Usando Ponto e Vírgula para abrir fácil no Excel Pt-BR
            ...rows.map(r => r.map(cell => `"${String(cell).replace(/"/g, '""')}"`).join(';'))
        ].join('\n');

        // Adiciona a marca de ordem de bytes para o Excel reconhecer UTF-8
