...
const PerformanceDetailModal: React.FC<PerformanceDetailModalProps> = ({ implantadorName, onClose }) => {
    ...
    return (
        <div aria-label="Performance Detail" className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-slate-950/70 backdrop-blur-sm animate-in fade-in duration-200">
            <div aria-label="Performance Detail" className="bg-white dark:bg-zinc-900 rounded-3xl shadow-2xl w-full max-w-5xl max-h-[90vh] flex flex-col overflow-hidden border border-slate-200 dark:border-zinc-800 ring-1 ring-white/10">
                {/* Header Premium */}
                <div aria-label="Performance Detail" className="px-8 py-6 border-b border-slate-100 dark:border-zinc-800 bg-white dark:bg-zinc-900 flex items-center justify-between shrink-0">
                ...
                {/* Body */}
                <div aria-label="Performance Detail" className="flex-1 overflow-y-auto p-8 bg-slate-50 dark:bg-zinc-950/30">
                    {loading ? (
                        ...
                        <div aria-label="Performance Detail" className="space-y-8">
                            ...
                            <div aria-label="Performance Detail" className="grid grid-cols-1 md:grid-cols-3 gap-6">
                                {/* Done Card */}
                                <div aria-label="Performance Detail" className="p-6 bg-white dark:bg-zinc-800 rounded-2xl border border-slate-200 dark:border-zinc-700/50 shadow-sm relative overflow-hidden group">
                                    ...
                                    <div aria-label="Performance Detail" className="mt-4 h-1.5 w-full bg-slate-100 dark:bg-zinc-700 rounded-full overflow-hidden">
                                ...
                                <div aria-label="Performance Detail" className="p-6 bg-white dark:bg-zinc-800 rounded-2xl border border-slate-200 dark:border-zinc-700/50 shadow-sm relative overflow-hidden group">
                                    ...
                                    <p className="text-xs font-bold text-orange-600 dark:text-orange-400 uppercase tracking-wider mb-2">Carga Atual (WIP)</p>
                                    <div aria-label="Performance Detail" className="flex items-baseline gap-2">
                                ...
                                {data.score_breakdown && (
                                    <div aria-label="Performance Detail" className="p-6 bg-gradient-to-br from-amber-50 to-orange-50 dark:from-amber-900/10 dark:to-orange-900/10 rounded-2xl border border-amber-100 dark:border-amber-900/30 shadow-sm relative overflow-hidden">
                                        ...
                                        <div aria-label="Performance Detail" className="grid grid-cols-2 gap-2">
                                            <div aria-label="Performance Detail" className="bg-white/60 dark:bg-black/20 p-2 rounded-lg flex justify-between items-center">
                            ...
                            {/* Detailed Table */}
                            <div aria-label="Performance Detail" className="bg-white dark:bg-zinc-800 rounded-2xl border border-slate-200 dark:border-zinc-700/50 overflow-hidden shadow-sm">
                                <div aria-label="Performance Detail" className="px-6 py-4 border-b border-slate-100 dark:border-zinc-700/50 flex justify-between items-center bg-slate-50/50 dark:bg-zinc-800/50">
                                            ...
                                            {data.stores.sort((a, b) => (b.is_done ? 1 : 0) - (a.is_done ? 1 : 0)).map(store => (
                                                <tr key={store.id} className="hover:bg-slate-50 dark:hover:bg-zinc-700/20 transition-colors group">
                                                    <td className="px-6 py-4">
                                                        <p className="font-bold text-slate-700 dark:text-zinc-200 group-hover:text-orange-600 dark:group-hover:text-orange-400 transition-colors">
                                                            {store.name}
                                                        </p>
                                                        {!store.is_done && (
