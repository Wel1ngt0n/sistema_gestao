...
interface MonitorLegacyProps {
    onEnableV2: () => void;
}

export default function MonitorLegacy({ onEnableV2 }: MonitorLegacyProps) {
    ...
    const [recommendedImpl, setRecommendedImpl] = useState<string | null>(null);
    ...
    const fetchRecommendation = () => {
        axios.get('http://localhost:5003/api/scoring/capacity')
            .then(res => {
                if (Array.isArray(res.data) && res.data.length > 0) {
                    // Sort by total_semester_points ASC (lowest first)
                    const sorted = [...res.data].sort((a: any, b: any) => (a.total_semester_points || 0) - (b.total_semester_points || 0));
                    if (sorted.length > 0) setRecommendedImpl(sorted[0].implantador);
                }
            })
            .catch(err => console.error("Error loading capacity logic", err));
    };
    ...
    return (
        <>
            <div className="fixed bottom-4 right-4 z-[100]">
                <button
                    onClick={onEnableV2}
                    className="bg-orange-600 text-white text-xs px-4 py-2 rounded-full shadow-xl border border-orange-500 hover:bg-orange-500 hover:scale-105 transition-all font-bold animate-pulse"
                >
                    ✨ Testar Monitor V2
                </button>
            </div>
                    ...
                    <div className={`mx-auto ${viewMode === 'kanban' ? 'w-full max-w-[calc(100vw-2rem)]' : 'max-w-[1920px]'} pb-20 animate-fade-in-up`}>
                        ...
                        {viewMode === 'table' && (
                            <MonitorTableView
                                ...
                                recommendedImplantador={recommendedImpl}
                            />
                        )}
